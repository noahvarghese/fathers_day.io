...
const NotRegisteredBuilder = <T extends Partial<FamilyNotRegisteredAttributes>>(
    options?: T
): FamilyNotRegisteredAttributes & T => Object.assign(EmptyNotRegisteredAttributes(), options);

@Entity({name: "family_not_registered"})
export default class FamilyNotRegistered extends BaseModel implements FamilyNotRegisteredAttributes {
    @Column()
